...
# Plotting view limits
zlim = [0, 250/1000]
xlim = [-60/1000, 60/1000]
ylim = [-60/1000, 60/1000]
...
#pAngles = 30+np.array([0, 60, 120, 180, 240, 300])
pAngles =  60+np.roll(np.array([-10, 10, 110, 130, 230, 250]), 1)
#pAngles = np.roll(np.array([-20, 20, 100, 140, 220, 260]), 0)
#pAngles = np.degrees(bAngles)
#pAngles = np.roll(np.arange(0, 360, 360/6), 1) + 30
#pAngles = np.array([180, 0])#, 240])
pAngles = np.radians(pAngles)
#pAngles = bAngles# + np.radians(30)
...
# Define length of legs
numLegs = pPos.shape[0]
legLower = np.full(numLegs, 60/1000)
legUpper = np.full(numLegs, 150/1000)


# Plotting view limits
zlim = [0, 250/1000]
xlim = [-200/1000, 200/1000]
ylim = [-200/1000, 200/1000]